...
class QuotationService:
            ...
            await self.db.execute(
                text("""
                    INSERT INTO quotations (
                        ...
                        advance_percent, delivery_percent, completion_percent,
                        margin_percent, reference_number,
    ...
    async def get_quotation(
        ...
        result = await self.db.execute(
            text("""
                SELECT id, reference_number, client_name, client_address,
                       service_option, advance_percent, delivery_percent,
                       completion_percent, margin_percent,
                       subtotal_sar, vat_sar, grand_total_sar,
        ...
        return QuotationResponse(
            ...
            advance_percent=float(row[5]),
            delivery_percent=float(row[6]),
            completion_percent=float(row[7]),
            margin_percent=float(row[8]),
